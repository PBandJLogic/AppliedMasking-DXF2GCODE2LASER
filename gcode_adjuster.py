...
class GCodeAdjuster:
    ...
    def setup_left_panel(self, parent):
        ...
        ttk.Button(file_frame, text="Load G-code File", command=self.load_gcode_file).pack(fill="x")
        ...
        ttk.Entry(left_expected_frame, textvariable=self.left_expected_x_var, width=10).pack(side="left", padx=(0, 5))
        ttk.Entry(left_expected_frame, textvariable=self.left_expected_y_var, width=10).pack(side="left")
        ...
        ttk.Entry(left_actual_frame, textvariable=self.left_actual_x_var, width=10).pack(side="left", padx=(0, 5))
        ttk.Entry(left_actual_frame, textvariable=self.left_actual_y_var, width=10).pack(side="left")
        ...
        ttk.Label(right_frame, text="Expected X, Y:", foreground="gray").pack(anchor="w")
        ...
        ttk.Entry(right_expected_frame, textvariable=self.right_expected_x_var, width=10).pack(side="left", padx=(0, 5))
        ttk.Entry(right_expected_frame, textvariable=self.right_expected_y_var, width=10).pack(side="left")
        ...
        ttk.Entry(right_actual_frame, textvariable=self.right_actual_x_var, width=10).pack(side="left", padx=(0, 5))
        ttk.Entry(right_actual_frame, textvariable=self.right_actual_y_var, width=10).pack(side="left")
        
        # Adjust button
        ttk.Button(parent, text="Adjust G-code", command=self.adjust_gcode).pack(fill="x", pady=(0, 10))
        ...
        results_text_scroll = ttk.Scrollbar(results_frame, orient="vertical", command=self.results_text.yview)
        self.results_text.configure(yscrollcommand=results_text_scroll.set)
        self.results_text.pack(side="left", fill="both", expand=True)
        results_text_scroll.pack(side="right", fill="y")
        
        # Save button
        ttk.Button(parent, text="Save Adjusted G-code", command=self.save_adjusted_gcode).pack(fill="x")
    ...
    def load_gcode_file(self):
        ...
        try:
            ...
            self.original_coords = self.parse_gcode_coordinates(self.original_gcode)
    ...
    def parse_gcode_coordinates(self, gcode):
        """Parse G-code and extract X,Y coordinates"""
        coords = []
        lines = gcode.split('\n')
        ...
        for line in lines:
            ...
            # Add coordinate if this line has movement
            if ('G0' in line or 'G1' in line) and (x_match or y_match):
    ...
    def plot_toolpath(self):
        ...
        if self.original_coords:
            ...
            orig_x, orig_y = zip(*self.original_coords) if self.original_coords else ([], [])
            self.ax.plot(orig_x, orig_y, 'b-', linewidth=2, label='Original', alpha=0.7)
            self.ax.scatter(orig_x, orig_y, c='blue', s=10, alpha=0.5)
        
        if self.adjusted_coords:
            # Plot adjusted toolpath
            adj_x, adj_y = zip(*self.adjusted_coords) if self.adjusted_coords else ([], [])
            self.ax.plot(adj_x, adj_y, 'r-', linewidth=2, label='Adjusted', alpha=0.7)
            self.ax.scatter(adj_x, adj_y, c='red', s=10, alpha=0.5)
    ...
    def adjust_gcode(self):
        ...
        try:
            ...
            left_expected = (float(self.left_expected_x_var.get()), float(self.left_expected_y_var.get()))
            left_actual = (float(self.left_actual_x_var.get()), float(self.left_actual_y_var.get()))
            right_expected = (float(self.right_expected_x_var.get()), float(self.right_expected_y_var.get()))
            right_actual = (float(self.right_actual_x_var.get()), float(self.right_actual_y_var.get()))
            ...
            actual_center, rotation_angle = self.calculate_corrections(left_actual, right_actual, expected_radius)
            
            # Calculate distances and errors
            left_distance = np.sqrt((left_actual[0] - actual_center[0])**2 + (left_actual[1] - actual_center[1])**2)
            right_distance = np.sqrt((right_actual[0] - actual_center[0])**2 + (right_actual[1] - actual_center[1])**2)
            ...
            self.adjusted_coords = self.apply_transformations(self.original_coords, actual_center, rotation_angle)
            
            # Generate adjusted G-code
            self.adjusted_gcode = self.generate_adjusted_gcode(self.original_gcode, actual_center, rotation_angle)
...
"""
    ...
    def calculate_corrections(self, left_actual, right_actual, expected_radius):
        ...
        chord_length = np.sqrt((right_actual[0] - left_actual[0])**2 + (right_actual[1] - left_actual[1])**2)
    ...
    def generate_adjusted_gcode(self, original_gcode, center, rotation_angle):
        ...
        for line in lines:
            ...
            if x_match or y_match:
                ...
                if x_match:
                    adjusted_line = re.sub(r'X[+-]?\d+\.?\d*', f'X{new_x:.6f}', adjusted_line)
                if y_match:
                    adjusted_line = re.sub(r'Y[+-]?\d+\.?\d*', f'Y{new_y:.6f}', adjusted_line)
    ...
    def save_adjusted_gcode(self):
        ...
        try:
            ...
            save_path = filedialog.asksaveasfilename(
                defaultextension=extension if hasattr(self, 'original_file_path') else ".nc",
            ...
            if save_path:
                ...
                messagebox.showinfo("Success", f"Adjusted G-code saved to:\n{save_path}")
